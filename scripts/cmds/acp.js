...
module.exports = {
  ...
  onStart: async function ({ event, api, commandName }) {
    ...
    api.sendMessage(`${msg}\nReply to this message with content: <add | del> <comparison | or "all"> to take action`, event.threadID, (e, info) => {
      global.GoatBot.onReply.set(info.messageID, { commandName, messageID: info.messageID, listRequest, author: event.senderID, unsendTimeout: setTimeout(() => {
  ...
  onReply: async function ({ message, Reply, event, api, commandName }) {
    ...
    if (args[0] === "add") {
      ...
      return api.sendMessage("Please select <add | del > <comparison | or \"all\">", event.threadID, event.messageID);
    }
